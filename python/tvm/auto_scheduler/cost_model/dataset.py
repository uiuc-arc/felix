...
class Dataset:
    def __init__(self):
        ...
        self.measure_records = (
            {}
        )  # Dict[LearningTask -> Tuple[List[MeasureInput], List[MeasureResult]]
    ...
    def random_split_within_task(
        self, train_set_ratio: float = None, train_set_num: int = None, shuffle_time: bool = False
        ...
        for task in self.features:
            ...
            if len(train_indices):
                ...
                train_set.load_task_data(
                    task, features[train_indices], train_throughputs, train_min_latency
                )

            if len(test_indices):
                test_throughputs = throughputs[test_indices]
                test_min_latency = self.min_latency[task] / np.max(test_throughputs)
                test_set.load_task_data(
                    task, features[test_indices], test_throughputs, test_min_latency
                )
    ...
    def extract_subset(self, tasks: List[LearningTask]) -> "Dataset":
        ...
        for task in tasks:
            if not (task in self.features):
                continue
            ret.load_task_data(
                task, self.features[task], self.throughputs[task], self.min_latency[task]
            )
        return ret
...
def make_dataset_from_log_file(log_files, min_sample_size, verbose=1):
    ...
    for filename in tqdm(log_files):
        ...
        if os.path.exists(cache_file):
            ...
            for task, (inputs, results) in measure_records.items():
                ...
                min_latency_ = min(float(sum(res.costs) / len(res.costs)) for res in results)
    ...
    for i, (task, feature) in enumerate(dataset.features.items()):
        if verbose >= 0:
            logger.info("No: %d\tTask: %s\tSize: %d" % (i, task, len(feature)))
